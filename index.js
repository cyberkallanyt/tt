...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Admin Baby BOT\n' 
            + 'ORG: BABY-BOT;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=7076355149:+91 751-045-7855\n' 
            + 'END:VCARD'
prefix = '$'
blocked = []   
limitawal = 99999
memberlimit = 0
cr = '*BABY-BOT THIS IS ALREADY VERIFIED*'

/******** OWNER NUMBER**********/
const ownerNumber = ["7076355149@s.whatsapp.net"] 
const pacarNumber = ["7076355149@s.whatsapp.net"]
/************************************/
...
itsmeiky.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `*Greetings, mownuse* @${num.split('@')[0]}\n*I will miss you* 🤭`
				let buff = await getBuffer(ppimg)
				itsmeiky.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	itsmeiky.on('message-new', async (iky) => {
          	...
          	let found = false
                    for (let lmt of _limit) {
                        if (lmt.id === sender) {
                            ...
                            if (limitCounts <= 0) return itsmeiky.sendMessage(from,`Your request limit has run out\n\n_Note : limit can be on get it by the way ${prefix}buylimit and by leveling up_`, text,{ quoted: iky})
		...
		if (isGroupAdmins) return reply('because you are a group admin, the bots wont kick you')
		itsmeiky.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin received")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`Group link detected sorry ${sender.split("@")[0]} you will be kicked from the group for another 5 seconds`)
		setTimeout( () => {
		...
		case 'transfer':
				...
				if (!q.includes('|')) return  reply(ind.wrongf())
                		...
                		addKoinUser('7076355149@s.whatsapp.net', fee)
                		reply(`*「 BABY-BOT 」*\n\nMoney transfer was successful\nFrom : +${sender.split("@")[0]}\nKe : +${tujuan}\n transfer amount: ${jumblah}\nTax : ${fee}`)
                		break
              case 'hai':
              case 'hi':
              case 'bot':
              ...
              case 'arjun':
				...
				if (!isPremium) return reply('Sorry you are not a premium user!')
				let ainecs = fs.readFileSync('./mp3/ehh2.mp3')
              ...
              case 'premam':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (!isPremium) return reply('Sorry you are not a premium user!')
				let ainezz = fs.readFileSync('./mp3/Iloveyou.mp3')
              ...
              case 'thaayoli':
				...
				itsmeiky.sendMessage(from, aineess, audio, {mimetype: 'audio/mp4', filename: `palepale2.mp3`, quoted: iky})
				await limitAdd(sender)
				break
              case 'what':
              case 'bots':
              ...
              case 'mowlee':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (!isPremium) return reply('Sorry you are not a premium user!')
				let aineesi = fs.readFileSync('./mp3/myname.mp3')
              ...
              case 'hallo':
				...
				fs.writeFileSync('./database/user/admin.json', JSON.stringify(adm))
				reply(`Successfully added bot admin wa.me/${adm} `)
				break
                case 'wakillist':
				itsmeiky.updatePresence(from, Presence.composing) 
				 // Fix Bug By ItsmeikyXSec404				
				...
				reply(`Made it to be premium wa.me/${premm} `)
				break
		case 'unpremium':
				if (!isOwner) return reply(ind.ownerb())
				premm = body.slice(11)
				...
				fs.writeFileSync('./database/user/premium.json', JSON.stringify(prem))
				reply(`Numbers have ended up being premium wa.me/${premm} `)
				break
                case 'premiumlist':
				itsmeiky.updatePresence(from, Presence.composing) 
				 // Fix Bug By ItsmeikyXSec404				
		...
		case 'calculator':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Sorry you got caught!')
				if (args.length < 1) return reply(`[❗] Send orders *${prefix}calculator [ Numbers ]*\nExample : ${prefix}calculator 12*12\n*NOTE* :\n• For Multiplication Using*\n• To Incremental Use +\n• For Reduction of Using -\n• For Sharing Using /`)
				const Math_js = require('mathjs')
				mtk = body.slice(12)
				if (typeof Math_js.evaluate(mtk) !== "number") {
					reply(`"${mtk}", Error!\n[❗] Send orders *${prefix}calculator [ Numbers ]*\nExample : ${prefix}calculator 12*12\n*NOTE* :\n• For Multiplication Using *\n• For Additional Use +\n• For Reduction of Using -\n• For Sharing Using /`)
				} else {
		...
		case 'buylimit':
				...
				if ( checkATMuser(sender) <= total) return reply(`sorry your money is not sufficient. please collect and buy later`)
				if ( checkATMuser(sender) >= total ) {
					confirmATM(sender, total)
					bayarLimit(sender, payout)
					
					await reply(`* |•SUCCESSFUL PAYMENT•| *\n\n*Sender* : Admin\n*Receiver* : ${pushname}\n*Purchase nominal* : ${payout} \n*Limit price* : ${koinPerlimit}/limit\n*The rest of your money* : ${checkATMuser(sender)}\n\Successful process with numbers payment\n${createSerial(20)}`)	} 
				break
		case 'buypremiumlimit':
				...
				if ( checkATMuser(sender) >= totalprem ) {
					...
					bayarLimit(sender, payout)
					await reply(`*「 SUCCESSFUL PAYMENT 」*\n\n*Sender* : Admin\n*Receiver* : ${pushname}\n*Nominal purchase* : ${payout} \n*Limit price* : ${koinpremPerlimit}/limit\n*The rest of your money* : ${checkATMuser(sender)}\n\n Successful process by number payment\n${createSerial(20)}`)
				} 
				break
		case 'giftlimit': 
				 // Fix Bug By ItsmeikyXSec404				
                 if (!isRegistered) return reply( ind.noregis())
				if (!isAdmin) return reply('*Only Admin & Owner only!*')
				const nomerr = args[0].replace('@','')
                		const jmla = args[1]
                		if (jmla <= 1) return reply(`the minimum gift limit is 1`)
                		if (isNaN(jmla)) return reply(`limit must be a number`)
                		if (!nomerr) return reply(`sorry wrong format\n input the correct parameters\nexample : ${prefix}giftlimit @7076355149 20`)
                		...
                		var found = false
                        	...
                        	if (found !== false) {
                            	...
                            	const result = `Gift quota limit was successful with NS: ${createSerial(20)} on ${moment().format('DD/MM/YY HH:mm:ss')}
							*「 GIFT QUOTA LIMIT 」*
		...
		case 'shortlink':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('myre nee ban aan!')
				itsmeiky.updatePresence(from, Presence.composing) 
		...
		case 'shortlink4':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('nee ban aan myre!')
				itsmeiky.updatePresence(from, Presence.composing) 
		...
		case 'shortlink6':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('ban aann punde nee!')
				itsmeiky.updatePresence(from, Presence.composing) 
		...
		/*case 'eyuth2':
 			 ...
 			 //  reply('p\n' + util.format({fontPath, inputPath, outputPath, tgl, hari, textnulis}))
 				 ...
 				 .on('exit', () => {
  			  itsmeiky.sendMessage(from, outputPath, image, {quoted: iky, caption : ' this dear, don't be lazy, honey'})
  			}))
  			  break*/
		case 'bplogo':
				 // Fix Bug By ItsmeikyXSec404				
				...
				buffer = await getBuffer(anu.result)
				itsmeiky.sendMessage(from, buffer, image, {caption: 'Here sis already finished..', quoted: iky})
				await limitAdd(sender)
				break
		case 'manoharam':
				 // Fix Bug By ItsmeikyXSec404				
		...
		case 'manoharam2':
				...
				itsmeiky.sendMessage(from, bh, image, {caption: 'Inna pidicho mownuse..', quoted: iky})
				await limitAdd(sender)
				break
		case 'manoharam3':
				 // Update By RzkyO & ItsmeikyXSec404				
				...
				itsmeiky.sendMessage(from, bh, image, {caption: 'dhaa pidicho saannam ready..', quoted: iky})
				await limitAdd(sender)
				break
		case 'text2gif':
				 // Update By RzkyO & ItsmeikyXSec404				
				...
				itsmeiky.sendMessage(from, bh, image, {caption: 'Here sis already finished..', quoted: iky})
				await limitAdd(sender)
				break
		case 'attp':
                 if (!isRegistered) return reply( ind.noregis())
		...
		case 'imgmaker':
				...
				buffer = await getBuffer(anu.result)
				itsmeiky.sendMessage(from, buffer, image, {caption: 'saadhanm inna pidicho..', quoted: iky})
				await limitAdd(sender)
				break
		case 'calendermaker':
				 // Fix Bug By ItsmeikyXSec404				
		...
		case 'shota':
				...
				if (isBanned) return reply('Sorry you got caught!')
				    try {
						...
						itsmeiky.sendMessage(from, buffer, image, {quoted: iky, caption: 'Remember! Love your mother'})
					} catch (e) {
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Sorry you got caught!')
				if (args.length < 1) return reply('Where is the text? dick ?')
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Sorry you got caught!')
				if (args.length < 1) return reply('where is the emoji? dick ?')
		...
		case 'logoepep': // Update By RzkyO & ItsmeikyXSec404 
				...
				itsmeiky.sendMessage(from, buffer, image, {quoted: iky, caption: 'Here is the result, sis...'})
				await limitAdd(sender)
				break
		case 'logoepep2': // Update By RzkyO & ItsmeikyXSec404 
				...
				itsmeiky.sendMessage(from, buffer, image, {quoted: iky, caption: 'Here is the result...'})
				await limitAdd(sender)
				break
		...
		case 'pubglogo': // Update By RzkyO & ItsmeikyXSec404				
				...
				buffer = await getBuffer(anu.result.url)
				itsmeiky.sendMessage(from, buffer, image, {quoted: iky, caption: 'inna pidicho boss...'})
				await limitAdd(sender)
				break
		...
		case 'play':   
				...
				if (anu.error) return reply(anu.error)
				infomp3 = `*Song Found!!!*\nJudul : ${anu.result.title}\nSource : ${anu.result.source}\nSize : ${anu.result.size}\n\n*WAIT AGAIN SENDED PLEASE DO NOT SPAM*`
		...
		case 'avengers':
				...
				itsmeiky.sendMessage(from, aruga, image, {caption: 'Here sis', quoted: iky})
				await limitAdd(sender)
				break 
		case 'summer':
				 // Fix Bug By ItsmeikyXSec404				
		...
		case 'neontext':
				...
				itsmeiky.sendMessage(from, aruga, image, {caption: 'itha brow', quoted: iky})
				await limitAdd(sender)
				break 
		case 'toxic13':
				 // Fix Bug By ItsmeikyXSec404				
		...
		case 'blood':
				...
				itsmeiky.sendMessage(from, aruga, image, {caption: 'got it brow', quoted: iky})
				await limitAdd(sender)
				break
		case 'firework':
				 // Fix Bug By ItsmeikyXSec404				
				...
				itsmeiky.sendMessage(from, arugazzz, image, {caption: 'got it brow', quoted: iky})
				await limitAdd(sender)
				break
		case 'lava':
				 // Fix Bug By ItsmeikyXSec404				
		...
		case 'setppbot':
					if (!isOwner) return reply(ind.ownerb())
					itsmeiky.updatePresence(from, Presence.composing) 
					if (!isQuotedImage) return reply(`Send pictures with captions ${prefix}setbotpp or image tag that has been sent`)
		...
		case 'bcgc':
					...
					if (isMedia && !iky.message.videoMessage || isQuotedImage) {
						...
						reply('Successful broadcast group')
					}
					break 
		case 'pinterest':
					 // Fix Bug By ItsmeikyXSec404				
			...
			case 'katailham':
				...
				case 'caklontong':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					...
					setTimeout( () => {
					itsmeiky.sendMessage(from, '*тЮ╕ Answer :* '+anu.jawaban+ '\n\nтАв Explanation: *'+ anu.keterangan+'*', text, {quoted: iky }) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
				...
				case 'tebakgambar':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					...
					setTimeout( () => {
					itsmeiky.sendMessage(from, '*тЮ╕ Answer :* '+anu.jawaban, text, {quoted: iky }) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					itsmeiky.sendMessage(from, '_10 Detik lagiтАж_', text) // ur cods
					}, 20000) // 1000 = 1s,
			 ...
			 case 'nsfwloli':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
						...
						itsmeiky.sendMessage(from, buffer, image, {quoted: shizuka, caption: 'Dont make ingredients for comli uncle'})
					    } catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					    }
		...
		case 'grouplist':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					itsmeiky.updatePresence(from, Presence.composing) 
					teks = `\`\`\`This is a list of the BABY-BOT group :\n\n\`\`\``
					no = 0
		...
		case 'daftar':
                			...
                			if(isNaN(umurUser)) return await reply('Age must be a number!!')
		...
		case 'mining':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await reply(`You're part of the prepayment, I will give as much*${one}Xp* for you`)
                 					     }else{
					const mining = Math.ceil(Math.random() * 1000000000000000000000000)
					addLevelingXp(sender, mining)
					await reply(`*Congratulations* ${pushname} you get *${mining}Xp*`)
					}
					await limitAdd(sender)
					break
		case 'bisakah':
		...
		case 'hobby':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					hobby = body.slice(1)
					const hob =['Cook','Membantu Atok','Mabar','Nobar','Sosmedtan','Membantu Orang lain','Nonton Anime','Nonton Drakor','Naik Motor','Nyanyi','Menari','Bertumbuk','Menggambar','Foto fotoan Ga jelas','Maen Game','Berbicara Sendiri']
...
◪ *ɞåɞʏ-ɞọṭ*
  ...
  ❏ Web Api: https://cyberkallan.herokuapp.com
  ----------------------------------
◪ *YOUR INFO*
  ❏ Prefix: 「  ${prefix}  」
  ❏ Name: ${pushname}
...
◪ *POWERED BY CYBERKALLAN*`
		...
		case 'menupict':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					buffer = await getBuffer(`https://i.ibb.co/smGmHDd/Pics-Art-03-05-10-36-31.jpg`)
		...
		case 'info':
					...
					teks = `*INFO OWNER*\n*Owner bot* : CYBERKALLAN\n*No Owner* : wa.me/7076355149\n*Ig owner* : www.instagram.com/arz_beats\n━━━━━━━━━━━━━━━━━━━━\n*INFO BOT*\n*Name bot* : ${me.name}\n*Number bot* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Total block contact* : ${blocked.length}\n*The bot is active on* : ${kyun(uptime)}\n*Fans page FB* : https://www.facebook.com/arjunarz\n*Type* : ${prefix}lpr _To report the bot admin via bot_\n*Type* : ${prefix}owner to contact our bot admin.`
					buffer = await getBuffer(`https://i.ibb.co/smGmHDd/Pics-Art-03-05-10-36-31.jpg`)
		...
		case 'snk':
					me = itsmeiky.user
					uptime = process.uptime()
					teks = `*BABY-BOT Terms & Conditions*\n1. The text and your WhatsApp username are stored on the server as long as the bot is active.\n2. Your data will be deleted when the bot is offline.\n3. We do not store images, videos, files, audios, and documents that you send.\n4. We never ask you to provide personal information.\n5. If you find a bug / error, please report it directly to the bot Owner.\n6. Just command 1x if the bot does not respond please repeat it again, if you repeat it does not respond, the bot is not active\n7. Do not spam, share virtex virus, phone calls, video calls, we will block you.\n8. Whatever you command this bot, *WE ARE NOT RESPONSIBLE!*\n\nTHANK YOU !~`
					buffer = await getBuffer(`https://i.ibb.co/smGmHDd/Pics-Art-03-05-10-36-31.jpg`)
					itsmeiky.sendMessage(from, buffer, image, {caption: teks, contextInfo:{mentionedJid: [me.jid]}})
					break
		case 'lpr':
					const bug = body.slice(5)
					if (pesan.length > 300) return itsmeiky.sendMessage(from, 'Sorry Text Too Long, Maximum 300 Text', msgType.text, {quoted: iky})
					...
					itsmeiky.sendMessage('7076355149@s.whatsapp.net', options, text, {quoted: iky})
					reply('Problems have been reported to the owner of Baby BOT, False or messy reports will not be responded to.')
					break
		case 'request':
					const cfrr = body.slice(8)
					if (cfrr.length > 300) return itsmeiky.sendMessage(from, 'Sorry Text Too Long, Maximum 300 Text', msgType.text, {quoted: iky})
					var nomor = iky.participant
					const ress = `*[REQUEST FEATURES]*\nNumber : @${nomor.split("@s.whatsapp.net")[0]}\nMessage : ${cfrr}`
							var options = {
							text: ress,
                         				contextInfo: {mentionedJid: [nomor]},
                     			}
					...
					itsmeiky.sendMessage('7076355149@s.whatsapp.net', options, text, {quoted: iky})
					itsmeiky.sendMessage('7076355149@s.whatsapp.net', options, text, {quoted: iky})
					reply('YOUR REQUEST HAS BEEN TO OWNER baby BOT, Fake Requests or mains will not be responded to.')
					break
		case 'blocklist': 
				...
				if (isBanned) return reply('Sorry you got caught!')
                                tels = body.slice(4)
                                if (args.length < 1) return reply('Brother, because of what?')
                                var ain = iky.participant
                                const tagzz = {
                                                text: `@${tagzz.split("@s.whatsapp.net")[0]} *MEDIUM AFK ${tels} DON'T DISTURB YES*`,
				...
				if (isBanned) return reply('Sorry you got caught!')
					...
					var wm = gh.split("|")[1];
					const pref = `Usage: \n${prefix}quotemaker teks|watermark\n\nEx :\n${prefix}quotemaker is an example | bicit`
                ...
                case 'truth':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Sorry you got caught!')
					const trut =["ഏത്് ടിവി ഷോയാണ് നിങ്ങൾ ഏറ്റവും വെറുക്കുന്നത്? കാരണം പറയുക!","നങ്ങൾൾ ഇതുവരെ ധരിച്ച ഏറ്റവും വൃത്തികെട്ട (നിങ്ങളുടെ അഭിപ്രായത്തിൽ) വസ്ത്രമെന്താണ്, എപ്പോഴാണ് നിങ്ങൾ ഇത് ധരിച്ചത്?",
"നിങ്ങളുടെ സുഹൃത്തിനെക്കുറിച്ച് നിങ്ങൾ ഇതുവരെ പറഞ്ഞതിൽ വച്ച് ഏറ്റവും മോശമായ കാര്യം (ഗോസിപ്പ്) എന്താണ്?",
"ക്കുറിച്ച്ുറിച്ച് ഏറ്റവും ലജ്ജാകരമായ കാര്യം എന്താണ്?",
"നിങ്ങളുടെ സുഹൃത്തിനെ സംബന്ധിച്ചിടത്തോളം ഏറ്റവും ലജ്ജാകരമായ കാര്യം എന്താണ്?",
"നങ്ങൾൾ മറ്റ് ആളുകളെ (എതിർലിംഗത്തിൽ) നോക്കുമ്പോൾ ആദ്യം കാണുന്നതെന്താണ്?",
"നിങ്ങൾ കണ്ണാടിയിൽ നോക്കുമ്പോൾ ആദ്യം നിങ്ങളുടെ മനസ്സിലേക്ക് വരുന്ന കാര്യം എന്താണ്?",
"ഇതുവരെ ഇതുവരെ ചെയ്തതിൽ വച്ച് ഏറ്റവും മണ്ടത്തരം എന്താണ്?",
"നിങ്ങൾ ഇതുവരെ ചെയ്തതിൽ വച്ച് ഏറ്റവും മണ്ടത്തരം എന്താണ്?",
"നിങ്ങളുടെ ഏറ്റവും വലിയ ഭയം എന്താണ്?",
"നിങ്ങൾക്ക് ഇതുവരെ ഉണ്ടായിട്ടില്ലാത്ത ഏറ്റവും മോശം പേടിസ്വപ്നം എന്താണ്?",
"പ്പോൾൾ വരെ നിങ്ങൾ ഓർക്കുന്ന ഏറ്റവും നിസാരമായ സ്വപ്നം എന്താണ്?",
"നിങ്ങൾ ആകുമെന്ന് നിങ്ങൾ സങ്കൽപ്പിച്ച ഏറ്റവും പരിഹാസ്യമായ ജോലി ഏതാണ്?",
"മരിക്കാൻ നിങ്ങൾ ഭയപ്പെടുന്നുണ്ടോ? എന്തുകൊണ്ട്?",
"നിങ്ങൾക്ക് ഒരു ബാങ്ക് കൊള്ളയടിച്ച് അതിൽ നിന്ന് രക്ഷപ്പെടാൻ കഴിയുമെങ്കി",
"ഈ മുറിയിലെ ആരുമായും നിങ്ങൾക്ക് ജീവിതം സ്വാപ്പ് ചെയ്യേണ്ടിവന്നാൽ, അത് ആരായിരിക്കും, എന്തുകൊണ്ട്?",
"നിങ്ങൾ വെറുത്തിരുന്ന ഒരു സമ്മാനം ഇഷ്ടപ്പെടുന്നതായി നിങ്ങൾ എപ്പോഴെങ്കിലും നടിച്ചിട്ടുണ്ടോ, നിങ്ങൾ ഇത് എന്തു ചെയ്തു?",
"നിങ്ങൾക്ക് ഈ മുറിയിൽ ആരെയെങ്കിലും ഡേറ്റ് ചെയ്യേണ്ടിവന്നാൽ, അത് ആരായിരിക്കും, എന്തുകൊണ്ട്?",
"നിങ്ങൾ എപ്പോഴെങ്കിലും മറ്റൊരാളെക്കുറിച്ച് ഒരു ശ്രുതി ആരംഭിച്ചിട്ടുണ്ടോ?",
"നിങ്ങളെ വഞ്ചിച്ചുവെന്ന് കണ്ടെത്തിയാൽ നിങ്ങൾ എന്തു ചെയ്യും?",
"ഈ മുറിയിലെ ഒരാളുമായി നിങ്ങൾക്ക് സംസാരിക്കുന്നത് നിർത്തേണ്ടിവന്നാൽ, അത് ആരായിരിക്കും, എന്തുകൊണ്ട്?",
"ഈ മുറിയിലെ ഏറ്റവും സെക്സി വ്യക്തി ആരാണ്?",
"നിങ്ങളുടെ വെബ് ചരിത്രത്തിൽ ആരെങ്കിലും കണ്ടാൽ ലജ്ജിക്കുന്നതെന്താണ്?",
"നിങ്ങൾ ലൈംഗിക ബന്ധത്തിൽ ഏർപ്പെട്ട വിചിത്രമായ സ്ഥലം എവിടെയാണ്?"]
					const ttrth = trut[Math.floor(Math.random() * trut.length)]
					truteh = await getBuffer(`https://i.ibb.co/0rNt7NT/todscarp.jpg`)
		...
		case 'dare':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					const dare =["Send a message to your ex and say 'I still like you'",
  "call Crush / girlfriend now and ss to the player",
  "Pap to a member of the group",
  "Saying 'YOU ARE SO BEAUTIFUL DON'T FOLLOWING' to guys",
  "ss recent call whatsapp",
  "drop emoticon '  👉👌' every typing on gc / pc for 1 day",
  "send a voice note say can i call u baby?",
  "drop the song / quote, continue to tag the appropriate member for that quote",
  "use Sule photos until 3 days",
  "type in the local language 24 pushname",
  "Change the name to 'I'm a cute child, Luna' for 5 pushnames",
  "chat to contact wa the order according to your% battery, keep telling him 'i lucky to hv you'",
  "Prank chat ex and say 'i love u, pgn back'",
  "voice record read surah al-kautsar",
  "said 'I hv crush on you, do you want to be my girlfriend or not?' to the opposite sex you last chat (hand over on wa / tele), wait for him to reply, if already ss drop here ",
  "state your boyfriend type!",
  "snap / post girlfriend / crush photos",
  "shouted gajelas then send using vn here",
  "Pap your face then send it to one of your friends",
  "send your photo with a caption, I'm an adopted child",
  "shouted using harsh words while Vn then sent it here",
  "shouted 'anjimm gabutt anjimmm' in front of your house ",
  "change the name to 'BOWO' for 24 pushnames",
  "Pretend to be possessed, for example: being possessed by maung, possessed by grasshoppers, possessed by a refrigerator, etc."]
					const der = dare[Math.floor(Math.random() * dare.length)]
					tod = await getBuffer(`https://i.ibb.co/0rNt7NT/todscarp.jpg`)
		...
		case 'ssweb':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					if (args.length < 1) return reply('Wheres the url, sis?')
					teks = body.slice(7)
					reply(ind.wait())
		...
		case 'ocr': 
				...
				if (isBanned) return reply('Sorry you got caught!')
					if ((isMedia && !iky.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(`*Send photos with captions ${prefix}ocr*`)
					}
					await limitAdd(sender)
					break
			case 'stickergif':
		...
		case 'ithpara':
				...
				if (!isPremium) return reply('Sorry you are not a premium user!')
				if (isBanned) return reply('Sorry you got caught!')
				if (args.length < 1) return itsmeiky.sendMessage(from, 'The language code is required!!', text, {quoted: iky})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return itsmeiky.sendMessage(from, 'Which text do you want to make a voice for? the voice of the devil huh?', text, {quoted: iky})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('The text is mostly satanic!! 😤')
		...
		case 'setprefix':
					...
					if (!isOwner) return reply(ind.ownerb())
					prefix = args[0]
					reply(`*Prefix successfully converted into* : ${prefix}`)
					break 
		case 'setlimit':
		case 'addlimit':
					if (args.length < 1) return
					if (!isOwner) return reply(ind.ownerb())
					limitawal = args[0]
					reply(`*The limit has been successfully changed to* : ${limitawal}`)
					break 
		case 'setlimitt':
		case 'addlimitt':
					if (args.length < 1) return
		...
		case 'tiktokstalk':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Sorry you have missed!')
				try {
						if (args.length < 1) return itsmeiky.sendMessage(from, '* wheres the username sis?', text, {quoted: iky})
				...
				break
		case 'sayang':
					itsmeiky.sendMessage(from, 'What my dear? 🥰',MessageType.text, { quoted: iky} )
					break
		case 'gelud':
					itsmeiky.sendMessage(from, 'Kuylah, get my money..',MessageType.text, { quoted: iky} )
					break
		case 'bot':
					itsmeiky.sendMessage(from, 'Yes bro? \nDont forget to register brow ^ _ ^"\n If you have, it means that you have registered brow ^_^"',MessageType.text, { quoted: iky} )
					break
    case 'myre':
					itsmeiky.sendMessage(from, 'ninde thandhene vilik ^_^"',MessageType.text, { quoted: iky} )
					break
    case 'cyberkallan':
					itsmeiky.sendMessage(from, 'he is my boss ^_^"',MessageType.text, { quoted: iky} )
					break
    case 'termux':
					itsmeiky.sendMessage(from, 'i love termux^_^"',MessageType.text, { quoted: iky} )
					break
    case 'kcg':
					itsmeiky.sendMessage(from, 'kerla cyber guardians. the team for fighting against to cyber bulliing and child porn ^_^"',MessageType.text, { quoted: iky} )
					break
    case 'whatislove':
					itsmeiky.sendMessage(from, 'humens have that.but i dont have that. enik atre ariyu ^_^"',MessageType.text, { quoted: iky} )
					break
    case 'fuck':
					itsmeiky.sendMessage(from, 'ninde thandhene vilik ^_^"',MessageType.text, { quoted: iky} )
					break
    ...
    case 'kundan':
					...
					break
   case 'whoisyourboss':
					itsmeiky.sendMessage(from, 'arjun arz made me ^_^"',MessageType.text, { quoted: iky} )
					break
    case 'sex':
    case 'suck':
    case 'sexme':
    ...
    case 'kali':
    case 'wantsex':
					itsmeiky.sendMessage(from, 'njhan robot aan myre. nee pooy valla vyayum thulak^_^"',MessageType.text, { quoted: iky} )
					break
    case 'goodnight':
    case 'gdni8':
    case 'gdnight':
    case 'goodbye':
    case 'sweetdreams':
					itsmeiky.sendMessage(from, 'gdnight da . sweet dreams^_^"',MessageType.text, { quoted: iky} )
					break
    case 'gdmorg':
    case 'goodmorning':
    case 'morg':
    case 'gdmg':
					itsmeiky.sendMessage(from, 'very very goodmorning dear^_^"',MessageType.text, { quoted: iky} )
					break
    case 'kannapi':
    case 'kannappi':
					itsmeiky.sendMessage(from, 'nee ninne thanen endhina vilikane ^_^"',MessageType.text, { quoted: iky} )
					break
    case 'girlfriend':
    case 'wantagirl':
    case 'pennineveenam':
    case 'givemeagirl':
					...
					itsmeiky.sendMessage(from, 'pooda myre^_^"',MessageType.text, { quoted: iky} )
					break
		case 'hargaprem':
		case 'hargapremium':
					...
					break
		case 'buypremium':
					itsmeiky.sendMessage(from, 'Want to buy premiumm?\nPlease contact us : wa.me/7076355149',MessageType.text, { quoted: iky} )
					break
		case 'buypremunlimited':
					if (!isPremium) return reply('Sorry you are not a premium user!')
					itsmeiky.sendMessage(from, 'Congratulations you have subscribed forever 🎉',MessageType.text, { quoted: iky} )
					break
		case 'buyprem30day':
					if (!isPremium) return reply('Sorry you are not a premium user!')
					itsmeiky.sendMessage(from, 'Congratulations on your 30-day subscription',MessageType.text, { quoted: iky} )
					break
		case 'buyprem7day':
					if (!isPremium) return reply('Sorry you are not a premium user!')
					itsmeiky.sendMessage(from, 'Congratulations you have subscribed for 7 days',MessageType.text, { quoted: iky} )
					break
		case 'test':
		...
		case 'ping':
					...
					break
		case 'dai':
					itsmeiky.sendMessage(from, 'endho..',MessageType.text, { quoted: iky} )
					break
    case 'sugaano':

					itsmeiky.sendMessage(from, 'nee undallo parama sugam..',MessageType.text, { quoted: iky} )
					break
    case 'vaaave':
    case 'vaave':
    case 'mutthey':
    case 'chakkare':
    case 'bae':
					itsmeiky.sendMessage(from, 'endho.. parada baby',MessageType.text, { quoted: iky} )
					break
		case 'chunni':
		case 'akku':
		case 'vikram':
		case 'ranjith':
		case 'tom':
					itsmeiky.sendMessage(from, 'kundans of the year',MessageType.text, { quoted: iky} )
					break
		case 'sreedurga':
		case 'durga':
		case 'sreekutty':
		case 'sree':
					itsmeiky.sendMessage(from, 'the cute and beautiful girl i know in my data base',MessageType.text, { quoted: iky} )
					break
		case 'arya':
		case 'saniya':
		case 'saumya':
		case 'maneesha':
		case 'theertha':
					itsmeiky.sendMessage(from, 'kannapis of kcg',MessageType.text, { quoted: iky} )
					break
		case 'arjun':
		case 'arjunarz':
		case 'arju':
		...
		case 'arz':
		case 'aju':
					itsmeiky.sendMessage(from, 'the best hacker and full stack developer in kozhikode district.costributed in many of apps and websites,expert in python and nodejs.the owner of CYBERALLAN youtube',MessageType.text, { quoted: iky} )
					break
		case 'sorry':
		case 'uff':
		case 'paavam':
		...
		case 'shit':
					...
					itsmeiky.sendMessage(from, 'its alright baby',MessageType.text, { quoted: iky} )
					break
		...
		case 'blockk':
				...
				if (!isAdmin) return reply('*Only Admin bot*')
					...
					itsmeiky.sendMessage(from, `*Order Received, Blocking* ${body.slice(7)}@c.us`, text)
					break
		case 'unblockk':
					if (!isGroup) return reply(ind.groupo())
				if (!isAdmin) return reply('*Only Admin bot*')
				    itsmeiky.blockUser (`${body.slice(10)}@c.us`, "remove")
					itsmeiky.sendMessage(from, `*Order Received, Unblock* ${body.slice(9)}@c.us`, text)
					break
		case 'block':
				 itsmeiky.updatePresence(from, Presence.composing) 
				 itsmeiky.chatRead (from)
		...
		case 'leave':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					...
					setTimeout( () => {
					itsmeiky.updatePresence(from, Presence.composing) 
					itsmeiky.sendMessage(from, 'mownuse bye..See you later 👋', text) // ur cods
					}, 0)
					break
		case 'bc': 
					if (!isOwner) return reply(ind.ownerb()) 
					...
					if (isMedia && !iky.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							itsmeiky.sendMessage(_.jid, buff, image, {caption: `*「 ORDER BROADCAST 」*\n\n${body.slice(4)}`})
						}
						reply('*Success broadcast* ')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「 BROADCAST BABY BOT 」*\n\n${body.slice(4)}`)
						}
						reply('*Success broadcast* ')
					}
					break
		case 'bcc': 
				if (!isAdmin) return reply('*Only Admin bot*')
					...
					if (isMedia && !iky.message.videoMessage || isQuotedImage) {
						...
						buff = await itsmeiky.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							itsmeiky.sendMessage(_.jid, buff, image, {caption: `*「 MESSAGE BROADCAST 」*\n\n${body.slice(4)}`})
						}
						reply('*Success broadcast* ')
					} else {
						for (let _ of anu) {
		...
		case 'setpp': 
					...
					reply('*Successfully changing the group icon*')
					break				
		case 'add':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					...
					if (args[0].startsWith('08')) return reply('Use the country code sis')
					try {
		...
		case 'group':
            ...
            case 'developer':
                  ...
                  itsmeiky.sendMessage(from, 'Thats my owner number >_<, dont spam or Ill block you',MessageType.text, { quoted: iky} )
					break    
           /*case 'setname':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
           ...
           /*case 'demote':
			...
			/*case 'promote':
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (iky.message.extendedTextMessage === undefined || iky.message.extendedTextMessage === null) return reply('*The target tag that you want to make the group admin!*')
					mentioned = iky.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `*Congratulations* 🥳 *You move up to be a group admin* 🎉 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						itsmeiky.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`*Congratulations* 🥳 @${mentioned[0].split('@')[0]} *You move up to be a group admin* 🎉`, mentioned, true)
						itsmeiky.groupMakeAdmin(from, mentioned)
					}
					break*/
		case 'kick':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `*Asek ration kick, otw kick* 🤭 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
		...
		case 'toimg':
				...
				if (!isQuotedSticker) return reply('*Reply/Tag sticker!*')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						itsmeiky.sendMessage(from, buffer, image, {quoted: iky, caption: '*inna pidicho* '})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
                 case 'simi':
					if (args.length < 1) return reply('Wheres the text, sis?')
					teks = body.slice(5)
		...
		case 'simih':
					...
					if (Number(args[0]) === 1) {
						if (isSimi) return reply('*The simi feature was active before*')
						samih.push(from)
		...
		case 'nsfw':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (args.length < 1) return reply('Activate press 1, Deactivate press ')
					if (Number(args[0]) === 1) {
		...
		case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ SUCCSESS° Baby-bot ❭activate the welcome feature in the group')
					} else if (Number(args[0]) === 0) {
                 ...
                 case 'antilink':
					...
					if (Number(args[0]) === 1) {
						if (isAntilink) return reply('babybot-Anti link group is active')
						antilink.push(from)
						fs.writeFileSync('./database/group/antilink.json', JSON.stringify(antilink))
						reply('Successfully activate anti link group in this group ✔️')
						itsmeiky.sendMessage(from,`BABY-BOT- Attention to all active anti link group members. If you send a group link, you will be kicked from the group`, text)
					} else if (Number(args[0]) === 0) {
						if (!isAntilink) return reply('Anti link group mode has been disabled')
						antilink.splice(from, 1)
						fs.writeFileSync('./database/group/antilink.json', JSON.stringify(antilink))
						reply('Successfully disable anti link group in this group ✔️')
					} else {
		...
		case 'clone':
			...
			if (body.startsWith(`${prefix}${command}`)) {
                  ...
                  reply(`Maaf *${pushname}*, Command *${prefix}${command}* Not Listed Inside *${prefix}menu*!`)
